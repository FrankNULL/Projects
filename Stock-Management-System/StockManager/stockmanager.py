...
class stackedExample(QWidget):
    def __init__(self):
        ...
        self.leftlist.insertItem(4, 'Generate Invoice')
        ...
        self.setGeometry(1000,1350, 600, 600)
    ...
    def stack5UI(self):
        layout = QVBoxLayout()
        self.gib = QPushButton()
        self.gib.setText("Generate Invoice")
        self.gib.clicked.connect(self.generate_invoice)
    
        self.input = QWidget()
        self.inputLayout = QFormLayout()
        self.invoice_name = QLineEdit()
        self.inputLayout.addRow("Name:", self.invoice_name)
        self.invoice_address = QLineEdit()
        self.inputLayout.addRow("Address:", self.invoice_address)
        self.invoice_contact = QLineEdit()
        self.inputLayout.addRow("Contact:", self.invoice_contact)
        self.input.setLayout(self.inputLayout)

        self.items = QWidget()
        self.itemsLayout = QFormLayout()
        self.item = QComboBox()
        self.item.addItem('One')
        self.item.addItem('Two')
        self.item.addItem('Three')
        self.item.addItem('Four')

        self.itemsLayout.addRow("Item:", self.item)
        self.item_price = QLineEdit()
        self.item_quantity = QLineEdit()
        
        self.itemsLayout.addRow("Price:", self.item_price)
        self.itemsLayout.addRow("Quantity:", self.item_quantity)
        ...
        self.tree.setItem(0, 0, QtWidgets.QTableWidgetItem('Item Name'))
        self.tree.setItem(0, 1, QtWidgets.QTableWidgetItem('Price per unit'))
        self.tree.setItem(0, 2, QtWidgets.QTableWidgetItem('Quantity'))
        self.tree.setItem(0, 3, QtWidgets.QTableWidgetItem('Line Total'))

        self.aib = QPushButton()
        self.aib.setText("Add item")
        self.aib.clicked.connect(self.add_items)


        self.siib = QPushButton()
        self.siib.setText("Show Invoice Items")
        self.siib.clicked.connect(self.show_invoice_items)

        self.show_invoice_items
        self.itemsLayout.addRow(self.aib)
        self.itemsLayout.addRow(self.siib)
        self.itemsLayout.addRow(self.tree)
        self.items.setLayout(self.itemsLayout)

        layout.addWidget(self.items)
        layout.addWidget(self.input)
        
        layout.addWidget(self.gib)
        # layout.addWidget(self.lbl3)
        # self.srb.clicked.connect(self.show_search)
    ...
    def generate_invoice(self):
        ...
        contact = self.invoice_contact.text()
        subtotal = 0.00
        for entry in invoice_list:
            subtotal += invoice_list[entry][3]
    ...
    def add_items(self):
            item_entry = [self.item.currentText(), self.item_price.text(), self.item_quantity.text(), float(self.item_price.text())* int(self.item_quantity.text())]
            invoice_list.append(item_entry)
            print(item_entry)
            i = len(invoice_list)+1
            self.tree.insertRow(i)
            self.tree.setItem(i, 0, QTableWidgetItem(str(item_entry[0])))
            self.tree.setItem(i, 1, QTableWidgetItem(str(item_entry[1])))
            self.tree.setItem(i, 2, QTableWidgetItem(str(item_entry[2])))
            self.tree.setItem(i, 3, QTableWidgetItem(str(item_entry[3])))
            self.tree.setRowHeight(i, 50)
    
    def show_invoice_items(self):
         if(len(invoice_list) != 0):
            for i in range(1,len(invoice_list)+1):
                item_entry = invoice_list[i-1]
                self.tree.insertRow(i)
                self.tree.setItem(i, 0, QTableWidgetItem(str(item_entry[0])))
                self.tree.setItem(i, 1, QTableWidgetItem(str(item_entry[1])))
                self.tree.setItem(i, 2, QTableWidgetItem(str(item_entry[2])))
